...
void ProcessExprBinaryAdd(DAST* dast,
    ...
    /* STUDENT CODE HERE. */
    if (c2PtrTotal > 0) {
        EmitADDI(T0, X0, child2->typeDecl->dataSize);
        EmitMUL(firstReg, firstReg, T0);
        EmitADD(S1, firstReg, S1);
    } else if (c1PtrTotal > 0){
        EmitADDI(T0, X0, child1->typeDecl->dataSize);
        EmitMUL(S1, S1, T0);
...
void ProcessExprBinaryGTEq(DAST* dast,
    ...
    /* STUDENT CODE HERE. */
    EmitADD(T0, S1, X0);
    EmitADD(T1, firstReg, X0);
    EmitXOR(T0, T0, T1);
    EmitSEQZ(T0, T0);
    if ((c1PtrTotal > 0 && c2PtrTotal > 0) && (child1->typeDecl == child2->typeDecl)) {
        EmitSLTU(S1, S1, firstReg);
    } else {
        if(child1->typeDecl == uintType || child2->typeDecl == uintType){
            EmitSLTU(S1, S1, firstReg);
        } else {
            if (child1->typeDecl == intType || child2->typeDecl == intType) {
                EmitSLT(S1, S1, firstReg);
            } else {
                if (child1->typeDecl == ucharType || child2->typeDecl == ucharType) {
                    EmitSLTU(S1, S1, firstReg);
                } else {
                    if (child1->typeDecl == charType || child2->typeDecl == charType) {
                        EmitSLT(S1, S1, firstReg);
                    } else {
                        EmitADDI(S1, X0, 1);
                    }
...
void ProcessExprBinaryGT(DAST* dast,
    ...
    /* STUDENT CODE HERE. */
    if ((c1PtrTotal > 0 && c2PtrTotal > 0) && (child1->typeDecl == child2->typeDecl)) {
        EmitSLTU(T5, firstReg, S1);
        EmitSLTU(T1, firstReg, S1);
        EmitSLTU(T2, S1, firstReg);
    } else {
        if(child1->typeDecl == uintType || child2->typeDecl == uintType){
            EmitSLTU(T5, firstReg, S1);
            EmitSLTU(T1, firstReg, S1);
...
void ProcessExprCall(DAST* dast, char* startLabel, char* endLabel, int regBytes) {
    ...
    if (params->size > 0) {
        ...
        for (int i = 0; i < params->size; i++) {
            /* STUDENT CODE HERE */
            DAST* childI = params->children[i];
            Dispatch(childI, startLabel, endLabel, regBytes);
            if (sizes[i] == 4) {
